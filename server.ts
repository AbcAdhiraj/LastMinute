...
function triggerGlobalRescheduling(uid: string = 'default'): void {
  updateDb((db) => {
    // 1. Clear all dynamically scheduled deep work study blocks
    db.scheduledSessions = db.scheduledSessions.filter(s => !(s.status === 'scheduled' && s.id.startsWith('sess_dyn_')));
    db.calendarEvents = db.calendarEvents.filter(e => !(e.category === 'deep_work' && e.id.startsWith('evt_sess_dyn_')));
 ...
 */
async function resolveMissedSessionsAndReschedule(uid: string): Promise<boolean> {
  let changed = false;
  const now = new Date();
...
app.post('/api/gmail-commitments/import/:id', (req: Request, res: Response) => {
  try {
    ...
    updateDb((db) => {
      ...
      if (idx !== -1) {
        ...
        if (ext) {
          ...
          // Create explicit fixed calendar event and scheduled session for the imported commitment
          const blockEnd = new Date(ext.deadline);
          const blockStart = new Date(blockEnd.getTime() - (ext.estimatedEffort * 3600 * 1000));
          const sessId = `sess_import_${importedTask.id}_${Date.now()}`;
          ...
          db.scheduledSessions.push({
            ...
            duration: ext.estimatedEffort,
            status: 'scheduled'
          });

          db.calendarEvents.push({
            id: `evt_${sessId}`,
            title: `Focus: ${importedTask.title}`,
            start: blockStart.toISOString(),
            end: blockEnd.toISOString(),
            category: 'deep_work',
            taskId: importedTask.id
    ...
    // Schedule the newly imported task dynamically
    triggerGlobalRescheduling(uid);
...
async function evalSingleTaskRisk(taskId: string, uid: string = 'default'): Promise<RiskAssessment> {
  ...
  const deadline = new Date(task.deadline);
   ...
   const hrsLeftToDeadline = (deadline.getTime() - new Date().getTime()) / (1000 * 60 * 60);
  ...
  const missedHrs = db.scheduledSessions
    .filter(s => s.taskId === taskId && s.status === 'missed')
    .reduce((sum, s) => sum + s.duration, 0);
  ...
  if (missedHrs > 0) {
    probability = Math.max(0, probability - (missedHrs * 10));
    if (status === 'on_track') status = 'at_risk';
    reason += ` Additionally, you have missed ${missedHrs}h of scheduled study sessions.`;
  }
...
app.post('/api/self-heal', async (req: Request, res: Response) => {
  try {
    ...
    for (const task of tasksToHeal) {
      ...
      // Recalculate Risk assessment for this task (only if not a manual boost)
