...
import { Task, CalendarEvent, ScheduledSession, GmailCommitment, Goal, Habit, Analytics, AppDatabase } from './types/index';
...
export default function App() {
  ...
  // User state
  const [currentUser, setCurrentUser] = useState<any>({ uid: 'default', email: 'raj@college.edu', displayName: 'Raj' });
  const [appDatabase, setAppDatabase] = useState<AppDatabase | null>(null);
  ...
  const handleLogOut = async () => {
    // No longer needed in local-only mode
    console.log("Log out clicked (no-op in local mode)");
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              ...
              <span className="text-xs text-black font-extrabold font-mono flex items-center gap-1.5 bg-[#dfbeff] px-3 py-1 rounded border-2 border-black neo-shadow-black-sm">
                ...
                Quote Of The Day: Keep Grinding For That SWE Offer
              </span>
            </div>
          </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          <div className="md:hidden">
                            <span className="text-[10px] uppercase tracking-wider font-extrabold text-black font-mono bg-white px-1 border border-black rounded">Caly</span>
                            <h4 className="text-xs font-black text-black">Your AI Assistant</h4>
                          </div>
                        </div>                         {/* Speech Bubble / Message Content */}
                        <div className="flex-1 space-y-2">
                          <div className="hidden md:block">
                            <span className="text-[9px] uppercase tracking-wider font-extrabold text-black font-mono bg-white px-1.5 py-0.5 border-2 border-black rounded shadow-[1.5px_1.5px_0px_0px_#000000]">Caly AI</span>
                          </div>
                          
                          <p className="text-xs text-black/90 leading-relaxed font-extrabold">
                            Hey Raj! Some of your tasks look very tight. Don't stress, we can easily fix it! Let's heal your schedule to make space.
                          </p>
                        ...
                        <div className="bg-[#ffffff] border-4 border-black rounded-xl p-5 space-y-4 neo-shadow-black">
                          ...
                          <div className="space-y-2.5">
                            {tasks.slice(0, 3).map((task) => {
                              ...
                              if (task.category === 'career' || task.category === 'meeting') cardBg = 'bg-[#f5f3ff] hover:bg-[#ede9fe]';
                              else if (task.category === 'study' || task.category === 'deep_work') cardBg = 'bg-[#f0f9ff] hover:bg-[#e0f2fe]';
                              else if (task.category === 'health') cardBg = 'bg-[#f0fdf4] hover:bg-[#dcfce7]';
                              else if (task.category === 'personal') cardBg = 'bg-[#fff1f2] hover:bg-[#ffe4e6]';

                              return (
                                <div key={task.id} className={`p-3 border-2 border-black rounded-lg flex items-center justify-between text-xs transition-all duration-100 neo-shadow-black-sm ${cardBg}`}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  <span className="text-[10px] font-mono font-extrabold tracking-wider text-black uppercase bg-white px-1.5 py-0.5 border-2 border-black rounded shadow-[1.5px_1.5px_0px_0px_#000000]">Caly AI</span>
